local interface_code = [=[
import os
from lupa import LuaRuntime

# Определяем текущую папку, где находится скрипт
current_dir = os.path.dirname(os.path.abspath(__file__))

# Создаем LuaRuntime
lua = LuaRuntime(unpack_returned_tuples=True)

# Формируем Lua-код с динамическим путем
lua_code = f"""
package.path = package.path .. ";{current_dir}/?.lua"
hamming = require("hamming")
print(hamming)
reedSolo = require("reedSolo")
block_parity = require("block_parity")
"""

# Выполняем Lua-код
lua.execute(lua_code)
# Получаем глобальные переменные из Lua-окружения
hamming = lua.globals().hamming
print(hamming)
print(dir(hamming))
reedSolo = lua.globals().reedSolo
block_parity = lua.globals().block_parity
...
# Lua-файл с кодом
interface_utils_lua_code_file = os.path.join(current_dir, "interface_utils.lua")

with open(interface_utils_lua_code_file, "r") as file:
    lua_content = file.read()

# Ищем Python-код внутри [[ ... ]]
import re

match = re.search(r"\[\[(.*?)\]\]", lua_content, re.DOTALL)
if match:
    python_code = match.group(1)  # Извлекаем содержимое блока [[ ... ]]
    exec(python_code)  # Выполняем Python-код
else:
    raise ValueError("Python code block not found in Lua file")

# Теперь можно использовать функции из interface_utils
    ...
    def get_answer(self, encode=True):
        if encode:
            ...
            if params == 0b00:
                ...
                lua_bits = lua.table(*bits)
                # Получаем закодированные данные
                # result = hamming.hamming_code(lua_bits)
                # print(f"result = hamming.hamming_code(lua_bits): {result}")
                result = list(hamming.hamming_code(lua_bits))
                print(f"result = list(hamming.hamming_code(lua_bits)): {result}")
            ...
            # Переход в случай метода Рида-Соломона
                ...
                bits = [int(bit) for bit in bits]
                lua_bits = lua.table(*bits)
                result = list(reedSolo.rs_encode_msg(lua_bits, 9))
                # Преобразуем список целых чисел в строку битов и устанавливаем в поле вывода
                result = "".join(str(bit) for bit in result)
            ...
            # Переход в случай метода блочного с проверкой чётности
                ...
                lua_bits = bits
                print(
                    f"params == 0b10 or params == 0b01 && lua_bits = lua.table(*bits): {lua_bits}"
                )
                result = block_parity.block_encode(lua_bits)
                print(
                    f"params == 0b10 or params == 0b01 && result = block_parity.block_encode(lua_bits): {result}"
                )
            ...
            if params == 0b00:
                ...
                result = list(hamming.hamming_decode(lua_bits))
                ...
                lua_bits = lua.table(*bits)
                corrected_message, corrected_ecc = reedSolo.rs_correct_msg(lua_bits, 9)
                corrected_message, corrected_ecc = list(corrected_message), list(
                    corrected_ecc
                )
                # Преобразуем список целых чисел в строку битов и расшифровываем ее
                ...
                lua_bits = bits
                result = str(block_parity.block_decode(lua_bits))
                result = self.decode_binary_string(result)
